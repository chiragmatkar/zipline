...
def calculate_results(host,
    ...
    if dividend_events is not None:
        ...
        # For testing purposes, we are building the dividend frame and injecting
        # it into the perf_tracker directly. Thus, we need to add the 'id' field
        #  here manually.
        dividend_frame['id'] = np.arange(
            len(dividend_frame)
        )
        dividend_frame.sort(['pay_date', 'ex_date']).set_index('id', drop=False)
